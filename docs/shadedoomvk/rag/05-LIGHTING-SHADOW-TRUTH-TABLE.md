...
Status: mixed active/partial/experimental  
...
PF-016 may replace collection/dedup internals only with selected-light equivalence evidence.
...
Sunlight can be represented as a cheap far-away directional-light approximation in dynamic-light data and has separate world/probe/lightmap roles. Actor sunlight visibility can use LevelMesh tracing and cached results.

PF-011 documents current intensity/attenuation conventions. PF-015 owns relevant visibility-cache invalidation.
...
| spot lights | active | angle/spot direction in light data |
| inverse-square/linear blend | active | map/light policy dependent |
...
Current classic/PBR equivalence uses implementation constants/approximations, including PBR brightness scaling and an ambient-sector-light PBR approximation. PF-011 extracts and documents current policy rather than silently recalibrating it.
